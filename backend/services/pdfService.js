...
const addFooter = (doc) => {
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    doc.fillColor(C.inkFaint).font("Helvetica").fontSize(7.5)
      .text(
        `Document généré automatiquement  ·  ${new Date().toLocaleString("fr-FR")}`,
        MARGIN, y, { width: CONTENT_W - 50, align: "left" }
      );
    doc.fillColor(C.inkFaint).font("Helvetica-Bold").fontSize(7.5)
...
const drawHeader = (doc, { title, subtitle, tag, tagColor }) => {
  ...
  doc.fillColor(C.surfaceCard).font("Helvetica-Bold").fontSize(25)
    .text(title, 22, 24, { characterSpacing: 0.8 });

  doc.fillColor(C.inkFaint).font("Helvetica").fontSize(10)
    .text(subtitle, 22, 60);

  if (tag) {
    const tc = tagColor === "blue" ? C.blue : C.accent;
    const bw = 68, bh = 24, bx = PAGE_W - bw - 22, by = 34;
    doc.roundedRect(bx, by, bw, bh, 5).fill(tc);
    doc.fillColor(C.surfaceCard).font("Helvetica-Bold").fontSize(10)
...
const infoGrid = (doc, items, columns = 2) => {
  ...
  rows.forEach((row) => {
    ...
    row.forEach((item, col) => {
      ...
      // Card
      doc.roundedRect(x, rowY, CW, CH, 7)
        .fillAndStroke(C.surfaceCard, C.border);

      // Label
      doc.fillColor(C.inkSoft).font("Helvetica").fontSize(7.5)
        .text(item.label.toUpperCase(), x + 12, rowY + 10, {
          width: CW - 24, characterSpacing: 0.3,
        });
...
const statRow = (doc, items) => {
  ...
  items.forEach((item, i) => {
    ...
    doc.fillColor(item.color || C.blue).font("Helvetica-Bold").fontSize(18)
      .text(val(item.value), x, rowY + 10, { width: W, align: "center" });

    doc.fillColor(item.color || C.blue).font("Helvetica").fontSize(7.5)
...
const table = (doc, title, columns, rows) => {
  ...
  columns.forEach((col, i) => {
    doc.fillColor(C.surfaceCard).font("Helvetica-Bold").fontSize(7.5)
      .text(col.toUpperCase(), MARGIN + i * CW + 8, hy + 9, {
        width: CW - 16, characterSpacing: 0.2,
      });
  });
  doc.y = hy + RH;
  ...
  rows.forEach((row, ri) => {
    ...
    row.forEach((cell, ci) => {
      doc.fillColor(ci === 0 ? C.inkMid : C.ink)
...
const generateBriefPDF = (brief) => {
  return createPdf("brief", (doc) => {
    ...
    infoGrid(doc, [
      { label: "Restaurant",     value: brief.restaurant || "Restaurant de Sèvres" },
      { label: "Manager",value: `${brief.managerPrenom || "Utilisateur"} ${brief.managerNom || ""}`.trim(),},      { label: "Date du rush",   value: formatDate(brief.dateRush) },
    ...
    statRow(doc, [
      { label: "CA — Heure +",  value: `${brief.heurePlusForte?.ca || 0} €`,     bg: C.blueBg,      color: C.blue },
      { label: "CA — 1/4 +",    value: `${brief.quartHeurePlusFort?.ca || 0} €`,  bg: C.blueBg,      color: C.blue },
      { label: "GC — Heure +",  value: brief.heurePlusForte?.gc || 0,             bg: C.greenBg,     color: C.green },

    ]);
    statRow(doc, [
      { label: "GC — 1/4 +",    value: brief.quartHeurePlusFort?.gc || 0,         bg: C.greenBg,     color: C.green },
      { label: "CA — Canal +",  value: `${brief.canalVentePlusFort?.ca || 0} €`,  bg: C.blueBg,      color: C.blue },
      { label: "GC — Canal +",  value: brief.canalVentePlusFort?.gc || 0,         bg: C.greenBg,     color: C.green },
    ]);
    infoGrid(doc, [
    ...
    statRow(doc, [
      { label: "Initiation", value: val(brief.tempsInitiation), bg: C.accentLight, color: C.accentDark },
      { label: "R2P",        value: val(brief.tempsR2P),        bg: C.accentLight, color: C.accentDark },
      { label: "LAD",        value: val(brief.tempsLAD),        bg: C.accentLight, color: C.accentDark },
    ]);
...
const generateDebriefPDF = (debrief) => {
  return createPdf("debrief", (doc) => {
    ...
    sectionTitle(doc, "Informations générales");
    infoGrid(doc, [
      { label: "Restaurant",   value: debrief.restaurant || "Restaurant de Sèvres" },
      {label: "Manager",value: `${debrief.managerPrenom || "Utilisateur"} ${debrief.managerNom || ""}`.trim(),},      { label: "Date du rush", value: formatDate(debrief.dateRush) },
      { label: "Service",      value: isMidi ? "Midi" : "Soir" },
    ]);

    sectionTitle(doc, "Analyse quart");
    statRow(doc, [
      { label: "Chiffre d'affaires", value: `${debrief.analyseQuart?.ca || 0} €`,  bg: C.blueBg,      color: C.blue },
      { label: "Transactions",       value: debrief.analyseQuart?.transaction || 0, bg: C.greenBg,     color: C.green },
      { label: "Staffing",           value: debrief.analyseQuart?.staffing || 0,    bg: C.accentLight, color: C.accentDark },
    ]);

    table(
      doc,
      "Analyse par tranche horaire",
      ["Créneau", "CA (€)", "TAC", "Staff.", "R2P", "Init L1", "Init L2", "Init L3"],
